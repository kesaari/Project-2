...
describe("getLanguagesStatistic", () => {
  ...
  it("Должна считать только актуальные отзывы", () => {
  ...
  it("Должна правильно подсчитывать языки", () => {
    const result = getLanguagesStatistic(data);
    expect(result["C"]).toBe(1);
    expect(result["JavaScript"]).toBe(2);
  });

  it("Должна возвращать пустой объект, если нет актуальных отзывов", () => {
  ...
  it("Должна игнорировать отзывы без указания языка", () => {
    const invalidData = [
      ...data,
      {
        firstName: "Test",
